...
def _save_tasks_from_semantic(db: Database, date_str: str, semantic_text: str):
    """Phase 1 出力の ```json ブロックをパースして tasks テーブルに保存する。"""
    match = re.search(r"```json\s*(\[.*?\])\s*```", semantic_text, re.DOTALL)
    if not match:
        return
    try:
        tasks = json.loads(match.group(1))
        if isinstance(tasks, list):
            db.upsert_tasks(date_str, tasks)
...
def analyze_today(db: Database | None = None) -> str:
    ...
    # タスクグループを抽出して保存
    _save_tasks_from_semantic(db, today, semantic_result)
